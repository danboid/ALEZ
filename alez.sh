# Arch Linux Easy ZFS (ALEZ) installation script 0.1
...
while [ "$dopart" == "y" ] || [ "$dopart" == "Y" ]; do
  ...
  read -p "ALL data on /dev/$blkdev will be lost? Proceed? (N/y) : " blkconf
...
do if (( $zpconf == 1 || $zpconf == 2 )); then
 if [ "$zpconf" == "1" ]; then 
   ...
   read -p "Enter the number of the first partition (1/2) : " zp1
   read -p "Enter the number of the second partition (2/2) : " zp2
   echo "Creating a mirror zpool..."
